...
<body>
	...
	<div class="body">
		<c:if test="${!empty veceri}">
			<h1>Pregled književnih večeri</h1><br>
			<table class="table-basic table-red">
				...
				<c:forEach items="${veceri}" var="v">
					<tr>
						...
						<td>${v.datum}</td>
						<td>${v.mesto}</td>
						<td>${v.maxPosetilaca}</td>
						<td>
							<form action="/IzdavackaKuca/radnik/otkaziVece" method="post">
								<input type="hidden" name="idVeceri" value="${v.idKnjizevnoVece}"/>
								<input type="submit" class="btn-red" value="Otkaži"/>
							</form>				
						</td>
					</tr>
				</c:forEach>
